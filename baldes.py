...
'''lista = [2, 2, 1, 4, 2, 3, 2, 2, 1, 2, 1, 1, 4, 4, 3, 2, 1]'''
lista = []
for j in range(5):
  lista.append(random.randint(1, 4))
'lista=[1, 3, 3, 4, 2, 1, 1, 3, 4, 1, 1, 4, 3, 2, 2, 2, 3, 4, 3, 4, 4, 4, 1, 3, 4, 3, 1, 2, 4, 1]'
print('LISTA')
print(lista)
tam_lista = len(lista)
...
'''def gpopulacao_i():
    ...
    cheio = np.zeros((1, tam_lista), dtype=int) 
    for x in range(tp):
        gene = []
        for j in range(tam_lista):
            x=False
            while x == False:
                sort = random.randint(0, tam_lista-1)
                if (cheio[0][sort] + lista[j]) <= lim:
                    cheio[0][sort]= cheio[0][sort] + lista[j]
                    x = True
            gene.append(sort)  
...
def gpopulacao_i():
    ...
    for x in range(tp):
        ...
        for j in range(tam_lista):
            ...
            value=b[index]
            if (cheio[0][value] + lista[j]) <= lim:
                    cheio[0][value]= cheio[0][value] + lista[j]
                    if (cheio[0][value]==lim):
                        b.remove(value)
            gene.append(value)  
        populacao.append(gene)
    return populacao 
...
def escolha(popul):
    global tp
    index1 = random.randint(74, (tp-1))
    index2 = random.randint(74, (tp-1))
